...
exports.buildForm = function(model, doc) {
  var field, value, view, form = '';
  for (i in model.fields) {
    ...
    view = {
      ...
      field: field,
      separator: field.separator ? ' separator="' + field.separator + '"' : '',
      value: value
    };
    // ignore rel:book for now
    if (field.type !== 'rel:book') {
      form += mustache.to_html(ddoc.templates.form[field.type], view);
    }
  }
  return form;
};
...
exports.db = (function() {
  ...
  db.createDoc = function(doc, options) {
    ...
    doc['jquery.couch.attachPrevRev'] = true;
  ...
  db.bulkCreate = function(docs, options) {
    ...
    for (var i in docs.docs) {
      docs.docs[i]._id = id;
      docs.docs[i]['jquery.couch.attachPrevRev'] = true;
